...
def classification_lstm(nb,npr,feature):
    predicted = []
    model = lstm(nb, npr)
    new_data = np.array([feature])
    new_data_scaled = scaler.transform(new_data.reshape(-1, 1)).reshape(-1, 1, 1)  # Reshape for LSTM input
    predictions = model.predict(new_data_scaled)
    for i in range(len(predictions)):
        threshold = 0.51
        predicted_class = 1 if predictions[i][0] > threshold else 0
        predicted.append(predicted_class)
    # print(predicted)
    return predicted

def lstm(data1, data2 ):
    all_data = np.concatenate([data1,data2])
    all_labels = np.concatenate([[0] * len(data1), [1] * len(data2)])
...
start   = time.time()
prediction = []
pred_one = classification_lstm(nb_shannon[0],pre_shannon[0],original_featureX[0])

prediction.append(pred_one)
for i in range(1,len(filtered_featureX)):
    pred_one = classification_lstm(nb_featureX[i],pre_featureX[i],filtered_featureX[i])
    prediction.append(pred_one)
end = time.time()
